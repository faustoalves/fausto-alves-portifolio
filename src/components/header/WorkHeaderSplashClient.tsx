...
const WorkHeaderSplashClient = ({
  ...
  return (
    <div className="w-full h-full absolute top-0 left-0 overflow-hidden z-1">
      <motion.div
        className={`${nextAnimation ? "w-1/2 h-1/2 lg:w-1/4 lg:h-1/3 rounded-tr-lg -bottom-px -left-px " : "w-full h-full bottom-0 left-0  rounded-tr-none"}  absolute bottom-0 left-0 transition-all duration-500 ease-in overflow-hidden z-1 flex items-center justify-center bg-linear-to-br ${color} shadow-2xl`}
      >
        <motion.div
